...
contract ForkTest2 is Test {
    ...
    function setUp() public {
        ...
        base_pool = ERC20(aerodromeRouter.poolFor(
            address(base_tokenP), address(base_tokenQ), false, defaultFactory)
        );
        ...
        l2LiquidityManager = new L2LiquidityManager(address(aerodromeRouter), feeReceiver, MIGRATION_FEE, endpointBase, delegate);
    ...
    function test_new_migrateV2Liquidity() public {
        ...
        deal(address(tokenP), user, 10_0 * pDec);
        deal(address(tokenQ), user, 25_0 * qDec);
        deal(user, 20 ether);
        ...
        (,,uint256 amountA, uint256 amountB) = abi.decode(_getWithdrawLiquidityData(entries), (address, address, uint256, uint256));
        ...
        if (address(base_tokenP) == baseWETH) {
            ...
            l2StandardBridge.finalizeBridgeETH{value: amountA}(address(liquidityMigration), address(l2LiquidityManager), amountA, "");
        }
        else {
            l2StandardBridge.finalizeBridgeERC20(address(base_tokenP), address(tokenP), address(liquidityMigration), address(l2LiquidityManager), amountA, "");
        }

        if (address(base_tokenQ) == baseWETH) {
            deal(l2messenger, amountB);

            l2StandardBridge.finalizeBridgeETH{value: amountB}(address(liquidityMigration), address(l2LiquidityManager), amountB, "");
        }
        else {
            l2StandardBridge.finalizeBridgeERC20(address(base_tokenQ), address(tokenQ), address(liquidityMigration), address(l2LiquidityManager), amountB, "");
        }
        vm.stopPrank();
        ...
        (uint256 valueIn, uint256 valueOut) = print_results(amountA, amountB, tokenPBefore, tokenQBefore, liqBefore, params);
    ...
    function _addV2Liquidity(address _user) internal returns(uint256 lpTokens) {
        ...
        uniswapV2Router.addLiquidity(address(tokenP), address(tokenQ), 100 * pDec, 100 * qDec, 0, 0, _user, block.timestamp);
